...
jQuery(document).ready(function () {
    ...
    $(".top-content").backstretch(
        "../img/284837124_1803181136543036_2766812211706276132_n.jpg"
    );
    /*
          Wow
      */
...
// const oldImgFile = document.getElementById("image-change").files;
...
function formatName(value) {
    ...
    for(let i = 0; i < nameArray.length; i++){
        if(nameArray[i] != ""){
            formatedName += nameArray[i].substring(0, 1).toUpperCase() + nameArray[i].substring(1, nameArray[i].length).toLowerCase() + " ";
        }
    }
...
function Validator(options) {
    ...
    function validate(inpElement, rule) {
        ...
        if (errMsg) {
            ...
            inpElement.removeClass("input-success");
        } else {
